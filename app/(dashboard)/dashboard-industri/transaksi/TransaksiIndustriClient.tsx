...
export default function TransaksiIndustriClient({
  ...
  useEffect(() => {
    if (currentList.length > 0) {
      // Keep selected if it's still in the current list
  ...
  const handleUploadPayment = async () => {
    ...
    try {
      ...
      if (!dbResult.success) {
        throw new Error(dbResult.error || 'Gagal menyimpan bukti transfer ke basis data.');
      }
      ...
      showToast(error.message || 'Terjadi kesalahan saat mengunggah bukti pembayaran.', 'error');
    } finally {
  ...
  return (
    <div className="w-full bg-bg-color min-h-screen text-text-main font-sans">
      <div className="p-8 max-w-7xl mx-auto">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 bg-transparent">
          <div>
            ...
            <p className="text-text-muted text-[14px] mt-0.5">Kelola pembayaran escrow dan pantau progres proyek industri Anda.</p>
          </div>

          <div className="flex items-center gap-4 bg-card-bg px-4 py-2 rounded-[30px] shadow-sm w-full md:w-auto border border-border-color">
            <div className="flex items-center bg-bg-color px-4 py-2 rounded-[20px] gap-2.5 flex-1 md:flex-none">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          {/* LEFT COLUMN: LIST TRANSAKSI (lg:col-span-5) */}
          <div className="lg:col-span-5 space-y-6">
            
            {/* Tabs */}
            <div className="flex bg-card-bg rounded-xl p-1 shadow-sm w-full border border-border-color">
              {tabs.map(tab => (
                <button
                  key={tab.key}
                  onClick={() => {
                  ...
                  className={`flex-1 py-2.5 rounded-lg font-semibold text-[13px] transition-all text-center ${
                    activeTab === tab.key
                      ...
                      : 'text-text-muted hover:text-text-main hover:bg-bg-color'
                  }`}
                >
                  {tab.label} ({tab.count})
                </button>
              ...
              {currentList.length > 0 ? (
                currentList.map(t => (
                  <div
                    ...
                    className={`bg-card-bg rounded-xl shadow-sm p-5 border cursor-pointer transition-all ${
                      selectedTrxId === t.id
                ...
                <div className="bg-card-bg rounded-xl shadow-sm p-12 text-center border border-border-color">
                  <div className="w-16 h-16 bg-bg-color rounded-full flex items-center justify-center mx-auto mb-4">
          ...
          {/* RIGHT COLUMN: DETAIL PANEL (lg:col-span-7) */}
            ...
            {selectedTransaksi ? (
              <div className="bg-card-bg rounded-2xl shadow-sm border border-border-color overflow-hidden">
                ...
                <div className="p-6 space-y-6">
                  ...
                  {/* Mitra Info */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-bg-color p-4 rounded-xl border border-border-color">
                    <div>
                    ...
                    <div>
                      ...
                      <strong className="text-[14px] text-text-main capitalize">{selectedTransaksi.status === 'lunas' ? 'Lunas / Selesai' : 'Sedang Berlangsung'}</strong>
                    </div>
                  </div>
                  ...
                  {selectedTransaksi.buktiTransfer && (
                    <div className="pt-4 border-t border-border-color space-y-3">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-bg-color rounded-xl border border-border-color gap-3">
                        <div className="flex items-center gap-3">
                  ...
                  <div className="pt-4 border-t border-border-color">
                    ...
                    {selectedTransaksi.history && selectedTransaksi.history.length > 0 ? (
                      <div className="bg-bg-color rounded-xl border border-border-color p-4 space-y-3">
              ...
              <div className="bg-card-bg rounded-2xl shadow-sm border border-border-color p-24 text-center">
                ...
                <div className="space-y-4">
                  {selectedTransaksi.history.map((h, i) => (
                    <div key={h.id} className="relative pl-6 border-l-2 border-slate-200 pb-4 last:border-0 last:pb-0">
                      ...
                      {h.pesan && (
                        <div className="bg-bg-color border border-border-color p-3 rounded-lg text-[12px] text-text-muted mt-1.5 italic">
                          "{h.pesan}"
                        </div>
                      )}
                    </div>
