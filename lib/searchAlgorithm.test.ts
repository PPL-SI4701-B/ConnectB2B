...
describe('searchAndClusterUmkm', () => {
  ...
  test('Pencarian dengan keyword dan kategori sekaligus', () => {
    const result = searchAndClusterUmkm(mockData, { keyword: 'Kue', kategoriList: ['Makanan'] });
    expect(result.length).toBe(1);
    expect(result[0].nama_usaha).toBe('Warung Bu Ani');

    const result2 = searchAndClusterUmkm(mockData, { keyword: 'Kue', kategoriList: ['Tekstil'] });
    expect(result2.length).toBe(0);
  });

  test('Pencarian berdasarkan nama produk', () => {
    const result = searchAndClusterUmkm(mockData, { keyword: 'Kain Katun' });
    expect(result.length).toBe(1);
    expect(result[0].nama_usaha).toBe('Maju Jaya Tekstil');
  });
  ...
  test('Clustering/Ranking berdasarkan relevansi', () => {
    ...
    const result = searchAndClusterUmkm(data, { keyword: 'Mesin' });
